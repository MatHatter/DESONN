...
SONN <- R6Class(
    ...
    initialize = function(input_size, hidden_sizes = NULL, output_size, Rdata = NULL, N,  lambda, ML_NN, dropout_rates = NULL, activation_functions_learn = NULL, activation_functions = NULL, method = init_method, custom_scale = custom_scale) {
        # }else{
        # initialize = function(input_size, output_size, Rdata = NULL, N,  lambda, ML_NN) {
        # }
...
self_organize = function(Rdata, labels, lr) {
    ...
    if (length(self$biases[[1]]) == 1) {
        # Print the components involved in the matrix function
        cat("self$biases[[1]]:", self$biases[[1]], "\n")
        cat("nrow(Rdata):", nrow(Rdata), "\n")
        cat("ncol(self$weights[[1]]):", as.matrix(self$weights), "\n")

        # Try to create the matrix and handle errors
        tryCatch({
            biases <- matrix(
                rep(self$biases[[1]], times = nrow(Rdata)),
                nrow = nrow(Rdata),
                ncol = ncol(as.matrix(self$weights[[1]])),
                byrow = TRUE
            )
            print("Matrix created successfully:")
            print(biases)
        }, error = function(e) {
            cat("Error in creating the matrix:\n", e$message, "\n")
        })

        # Single bias value
        biases <- matrix(rep(self$biases[[1]], times = nrow(Rdata)),
                         nrow = nrow(Rdata), ncol = ncol(as.matrix(self$weights[[1]])), byrow = TRUE)
    ...
    if(self$ML_NN){
        outputs <- tryCatch(
            {
                # First attempt: add bias directly
                Rdata %*% self$weights[[1]] + self$biases[[1]]
            },
            error = function(e) {
                tryCatch(
                    {
                        # Second attempt: add transposed bias
                        Rdata %*% self$weights[[1]] + t(self$biases[[1]])
                    },
                    error = function(e) {
                        tryCatch(
                            {
                                # Third attempt: reshape biases and add
                                result <- Rdata %*% self$weights[[1]]
                                result + matrix(self$biases[[1]], nrow = nrow(result), ncol = ncol(result), byrow = TRUE)
                            },
                            error = function(e) {
                                tryCatch(
                                    {
                                        # Fourth attempt: truncate biases to match dimensions
                                        result <- Rdata %*% self$weights[[1]]
                                        truncated_biases <- self$biases[[1]][1:ncol(result)]
                                        result + matrix(truncated_biases, nrow = nrow(result), ncol = ncol(result), byrow = TRUE)
                                    },
                                    error = function(e) {
                                        stop("Matrix multiplication or bias addition failed. Check dimensions of Rdata, weights, and biases.")
                                    }
                                )
                            }
                        )
    ...
    if (self$ML_NN) {
        ...
        for (layer in 2:self$num_layers) {
            ...
            if (length(self$biases[[layer]]) != 1 && length(self$biases[[layer]]) != ncol(self$weights[[layer]])) {
                warning("Number of biases does not match the number of columns in the weights matrix")
            }
...
print("print(str(outputs))")
    ...
    if (self$ML_NN) {
        ...
        if (ncol(Rdata) != ncol(outputs[[self$num_layers]])) {
            rep_factor <- ceiling((nrow(Rdata) * ncol(Rdata)) / length(outputs[[self$num_layers]]))
            replicated_outputs <- rep(outputs[[self$num_layers]], rep_factor)
            replicated_outputs <- replicated_outputs[1:(nrow(Rdata) * ncol(Rdata))]
            output_matrix <- matrix(replicated_outputs, nrow = nrow(Rdata), ncol = ncol(Rdata), byrow = FALSE)
            error_1000x10 <- Rdata - output_matrix
        } else {
            error_1000x10 <- Rdata - outputs[[self$num_layers]]
        }
    } else {
        if (ncol(Rdata) != ncol(outputs)) {
            rep_factor <- ceiling((nrow(Rdata) * ncol(Rdata)) / length(outputs))
            replicated_outputs <- rep(outputs, rep_factor)
            replicated_outputs <- replicated_outputs[1:(nrow(Rdata) * ncol(Rdata))]
            output_matrix <- matrix(replicated_outputs, nrow = nrow(Rdata), ncol = ncol(Rdata), byrow = FALSE)
        } else {
...
# print("print(str(error_1000x10))")
# print(str(error_1000x10))
...
#         # print("===========================")
#         # print(paste("layer", layer))
#         # print("Dimensions of self$weights[[layer + 1]]:")
#         # print(dim(self$weights[[layer + 1]]))
#         # print("Dimensions of errors[[layer + 1]] before:")
#         # print(dim(errors[[layer + 1]]))
#
#         # Ensure the number of columns in errors[[layer + 1]] matches the number of rows in self$weights[[layer + 1]]
#         if (ncol(errors[[layer + 1]]) != nrow(self$weights[[layer + 1]])) {
#             if (ncol(errors[[layer + 1]]) > nrow(self$weights[[layer + 1]])) {
#                 # Truncate columns of errors[[layer + 1]] to match the number of rows in self$weights[[layer + 1]]
#                 errors[[layer + 1]] <- errors[[layer + 1]][, 1:nrow(self$weights[[layer + 1]]), drop = FALSE]
#             } else {
#                 # Replicate columns of errors[[layer + 1]] to match the number of rows in self$weights[[layer + 1]]
#                 errors[[layer + 1]] <- errors[[layer + 1]][, rep(1:ncol(errors[[layer + 1]]), length.out = nrow(self$weights[[layer + 1]])), drop = FALSE]
#             }
#         }
#
#         # print("Dimensions of errors[[layer + 1]] after:")
#         # print(dim(errors[[layer + 1]]))
#
#         # Perform Matrix Multiplication with Exception Handling
#         tryCatch(
#             expr = {
#                 print("First multiplication attempt")
#                 if (nrow(errors[[layer + 1]]) == 1 && ncol(errors[[layer + 1]]) == 1) {
#                     errors[[layer]] <- self$weights[[layer + 1]] * errors[[layer + 1]]
#                 } else {
#                     errors[[layer]] <- errors[[layer + 1]] %*% self$weights[[layer + 1]]
#                 }
#             },
#             error = function(e) {
#                 print("Error in first multiplication attempt")
#                 tryCatch(
#                     expr = {
#                         print("Second multiplication attempt")
#                         if (nrow(errors[[layer + 1]]) == 1 && ncol(errors[[layer + 1]]) == 1) {
#                             errors[[layer]] <- t(self$weights[[layer + 1]]) * errors[[layer + 1]]
#                         } else {
#                             errors[[layer]] <- errors[[layer + 1]] %*% t(self$weights[[layer + 1]])
#                         }
#                     },
#                     error = function(e) {
#                         print("Error in second multiplication attempt")
#                         tryCatch(
#                             expr = {
#                                 print("Third multiplication attempt")
#                                 if (nrow(errors[[layer + 1]]) == 1 && ncol(errors[[layer + 1]]) == 1) {
#                                     errors[[layer]] <- errors[[layer + 1]] * t(self$weights[[layer + 1]])
#                                 } else {
#                                     errors[[layer]] <- t(errors[[layer + 1]]) %*% self$weights[[layer + 1]]
#                                 }
#                             },
#                             error = function(e) {
#                                 print("Error in third multiplication attempt")
#                                 tryCatch(
#                                     expr = {
#                                         print("Fourth multiplication attempt")
#                                         if (nrow(errors[[layer + 1]]) == 1 && ncol(errors[[layer + 1]]) == 1) {
#                                             errors[[layer]] <- t(errors[[layer + 1]]) * t(self$weights[[layer + 1]])
#                                         } else {
#                                             errors[[layer]] <- t(errors[[layer + 1]]) %*% t(self$weights[[layer + 1]])
#                                         }
#                                     },
#                                     error = function(e) {
#                                         stop("Matrix multiplication failed. Check dimensions of error and weights matrices.")
#                                     }
#                                 )
#                             }
#                         )
...
#         )
#
# # print("errors layerrrr")
# # print(str(errors[[layer]]))
#
#     }
#######################
    ...
    if (self$ML_NN) {
        ...
        for (layer in (self$num_layers - 1):1) {
            ...
            if (error_cols != weight_rows) {
                if (error_cols > weight_rows) {
                    errors[[layer + 1]] <- errors[[layer + 1]][, 1:weight_rows, drop = FALSE]
                } else {
                    errors[[layer + 1]] <- matrix(
                        rep(errors[[layer + 1]], length.out = error_rows * weight_rows),
                        nrow = error_rows,
                        ncol = weight_rows
                    )
                }
            ...
            if (error_rows != weight_cols) {
                if (error_rows > weight_cols) {
                    errors[[layer + 1]] <- errors[[layer + 1]][1:weight_cols, , drop = FALSE]
                } else {
                    errors[[layer + 1]] <- matrix(
                        rep(errors[[layer + 1]], length.out = weight_cols * ncol(errors[[layer + 1]])),
                        nrow = weight_cols,
                        ncol = ncol(errors[[layer + 1]])
                    )
                }
            }
    ...
    if(self$ML_NN){
        ...
        # Ensure the number of columns in errors[[1]] matches the number of rows in Rdata
        if (ncol(errors[[1]]) != nrow(Rdata)) {
            if (ncol(errors[[1]]) > nrow(Rdata)) {
                # Truncate columns of errors[[1]] to match the number of rows in Rdata
                errors[[1]] <- errors[[1]][, 1:nrow(Rdata), drop = FALSE]
            } else {
                # Replicate columns of errors[[1]] to match the number of rows in Rdata
                errors[[1]] <- errors[[1]][, rep(1:ncol(errors[[1]]), length.out = nrow(Rdata)), drop = FALSE]
            }
        }
...
if (ncol(errors[[1]]) == nrow(self$weights[[1]])) {
    self$weights[[1]] <- self$weights[[1]] - (lr * t(errors[[1]]) %*% Rdata)
} else if (nrow(t(errors[[1]])) == nrow(self$weights[[1]]) && ncol(t(errors[[1]])) < ncol(Rdata)) {
    self$weights[[1]] <- self$weights[[1]] - ((lr * errors[[1]] %*% Rdata))[, 1:ncol(self$weights[[1]])]
} else if (prod(dim(self$weights[[1]])) == 1) {
    # Handle scalar weight updates
    update_value <- lr * sum(errors[[1]] %*% Rdata)
    self$weights[[1]] <- self$weights[[1]] - update_value
} else {
    self$weights[[1]] <- self$weights[[1]] - (lr * apply(errors[[1]] %*% Rdata, 2, mean))
}



        # Ensure biases and colMeans have matching lengths
        ...
        self$weights <- self$weights - (lr * apply(t(error_1000x10) %*% Rdata, 2, mean)) # Update weights
        self$biases <- self$biases - (lr * colSums(error_1000x10))
    }
        ...
        for (layer in 2:self$num_layers) {
            ...
            if (ncol(self$weights[[layer]]) == ncol(hidden_outputs[[layer - 1]])) {
                if (all(weight_dim == result_dim)) {
                    self$weights[[layer]] <- self$weights[[layer]] - (lr * errors[[layer]] %*% (hidden_outputs[[layer - 1]]))
                } else {
                    cat("Dimensions mismatch, handling default case for weights.\n")
                }
            } else if (nrow(self$weights[[layer]]) == ncol(hidden_outputs[[layer - 1]]) && ncol(self$weights[[layer]]) < ncol(hidden_outputs[[layer - 1]])) {
                self$weights[[layer]] <- self$weights[[layer]] - colMeans((lr * errors[[layer]] %*% (hidden_outputs[[layer - 1]])))
            } else if (prod(weight_dim) == 1) {
                # Handle scalar weight updates
                update_value <- lr * sum(errors[[layer]] %*% t(hidden_outputs[[layer - 1]]))
                self$weights[[layer]] <- self$weights[[layer]] - update_value
            } else {
                self$weights[[layer]] <- self$weights[[layer]] - (lr * apply(errors[[layer]] %*% (hidden_outputs[[layer - 1]]), 2, mean))
            }
...
learn = function(Rdata, labels, lr, activation_functions_learn, dropout_rates_learn) {
    ...
    if (!olr) {
        ...
        if (!is.null(activation_functions_learn[[1]]) && activation_functions_learn[[1]] %in% c("sigmoid", "sigmoid_binary", "relu", "leaky_relu", "elu", "tanh", "swish", "softplus", "hard_sigmoid", "gelu", "selu", "mish", "maxout", "prelu", "binary_activation", "custom_activation", "custom_binary_activation", "softmax", "bent_identity")) {
            activation_function_learn <- get(activation_functions_learn[[1]])
        } else {
            ...
            predicted_output_learn <- tryCatch(
                {
                    if (!is.null(activation_function_learn)) {
                        # Attempt to compute the weighted sum plus bias, then apply the activation function
                        tryCatch(
                            {
                                # First attempt: add bias directly
                                result <- Rdata %*% self$weights[[1]] + self$biases[[1]]
                                activation_function_learn(result)
                            },
                            error = function(e) {
                                tryCatch(
                                    {
                                        # Second attempt: add transposed bias
                                        result <- Rdata %*% self$weights[[1]] + t(self$biases[[1]])
                                        activation_function_learn(result)
                                    },
                                    error = function(e) {
                                        tryCatch(
                                            {
                                                # Third attempt: reshape biases and add
                                                result <- Rdata %*% self$weights[[1]]
                                                result <- result + matrix(self$biases[[1]], nrow = nrow(result), ncol = ncol(result), byrow = TRUE)
                                                activation_function_learn(result)
                                            },
                                            error = function(e) {
                                                tryCatch(
                                                    {
                                                        ...
                                                        result <- Rdata %*% self$weights[[1]]
                                                        truncated_biases <- self$biases[[1]][1:ncol(result)]
                                                        result <- result + matrix(truncated_biases, nrow = nrow(result), ncol = ncol(result), byrow = TRUE)
                                                        activation_function_learn(result)
                                                    },
                                                    error = function(e) {
                                                        # Fifth attempt: extend biases to match dimensions
                                                        result <- Rdata %*% self$weights[[1]]
                                                        extended_biases <- rep(self$biases[[1]], length.out = ncol(result))
                                                        result <- result + matrix(extended_biases, nrow = nrow(result), ncol = ncol(result), byrow = TRUE)
                                                        activation_function_learn(result)
                                                    },
                                                    error = function(e) {
                                                        stop("Matrix multiplication, bias addition, or activation function application failed. Check dimensions of Rdata, weights, and biases.")
                                                    }
                                                )
                                            }
                                        )
                        ...
                        # If no activation function is provided, just compute the weighted sum plus bias
                        result <- tryCatch(
                            {
                                # First attempt: add bias directly
                                Rdata %*% self$weights[[1]] + self$biases[[1]]
                        ...
                        # Return result without activation function
                        result
                    }
                },
                error = function(e) {
                    stop("An unexpected error occurred during the computation. 817")
                }
            )

        } else {
            # Apply the activation function if it's not NULL
            if (!is.null(activation_functions[[2]])) {
                # Retrieve and apply the activation function
                predicted_output_learn <- tryCatch(
                    {
                        # Dynamically fetch the activation function
                        activation_function <- get(activation_functions[[2]], mode = "function", envir = environment())

                        # Debugging: Ensure activation_function is valid
                        if (is.null(activation_function)) {
                            stop("Activation function retrieved is NULL.")
                        }

                        # Adjust biases and compute the prediction
                        biases <- adjust_biases_layer_1(self$biases, self$weights, Rdata)
                        activation_function(Rdata %*% self$weights + biases)
                    },
                    error = function(e) {
                        stop("Error during computation with activation function: ", conditionMessage(e))
                    }
                )
            } else {
                # No activation function provided, perform linear computation
                predicted_output_learn <- tryCatch(
                    {
                        biases <- adjust_biases_layer_1(self$biases, self$weights, Rdata)
                        Rdata %*% self$weights + biases
                    },
                    error = function(e) {
                        stop("Error during linear computation: ", conditionMessage(e))
                    }
                )
            }
        ...
        # Combine y and X into a single dataframe with y as the first column
        dataset <- cbind(labels, data.frame(Rdata))

        # Optionally, you can set column names for X and y
        colnames(dataset) <- c("y", paste0("X", 1:input_size))

        responseName <- 'y'
        predictorNames <- c("X1", "X2", "X3", "X4", "X5", "X6", "X7", "X8", "X9", "X10", "X11", "X12")

        olr1 <<- suppressMessages(olr(dataset, responseName, predictorNames, adjr2 = FALSE))

        coefficients_olr <- olr1$coefficients

        # Use the degrees of freedom from your model
        df <- olr1$df[2]

        # Extract the t-values from coefficients_olr
        t_values <<- coefficients_olr[2:(length(predictorNames)+1), "t value"]

        # Calculate the p-values from the t-values
        p_values <<- 2 * (1 - pt(abs(t_values), df))

        # Subtract p-values from 1 to get weights
        weights <- 1 - p_values

        # Normalize the weights so they sum to 1
        weights <<- weights / sum(weights)

        # Apply the weights to each column of Rdata
        Rdata_olr <- Rdata %*% diag(weights)

        if(ML_NN){
            if(!is.null(activation_function_learn)){
                predicted_output_learn <- activation_function_learn(Rdata_olr %*% self$weights[[1]] + self$biases[[1]])
            }else{
                predicted_output_learn <- (Rdata_olr %*% self$weights[[1]]) + self$biases[[1]]
            }
        }else{
            if(!is.null(activation_function_learn)){
                predicted_output_learn <- activation_function_learn(Rdata_olr %*% self$weights + self$biases)
            }else{
                predicted_output_learn <- (Rdata_olr %*% self$weights) + self$biases
            }
        }
    ...
    if (ML_NN) {
        ...
        for (layer in 2:self$num_layers) {
            ...
            if (!is.null(activation_functions_learn[[layer]]) && activation_functions_learn[[layer]] %in% c("sigmoid", "sigmoid_binary", "relu", "leaky_relu", "elu", "tanh", "swish", "softplus", "hard_sigmoid", "gelu", "selu", "mish", "maxout", "prelu", "binary_activation", "custom_activation", "custom_binary_activation", "softmax", "bent_identity")) {
    ...
    if (ML_NN) {
        ...
        for (layer in 1:self$num_layers) {
            if (layer == 1) {
                # For the first layer, replicate or truncate predicted_output_learn_hidden[[1]]
                if (ncol(predicted_output_learn_hidden[[1]]) < ncol(labels)) {
                    # Replicate predicted_output_learn_hidden[[1]] to match the dimensions of labels
                    replicated_predicted_output_learn_hidden <- matrix(
                        rep(predicted_output_learn_hidden[[1]], each = ncol(labels) / ncol(predicted_output_learn_hidden[[1]])),
                        nrow = nrow(predicted_output_learn_hidden[[1]]),
                        ncol = ncol(labels),
                        byrow = FALSE  # Replicate by columns
                    )
                } else if (ncol(predicted_output_learn_hidden[[1]]) > ncol(labels)) {
                    # Truncate predicted_output_learn_hidden[[1]] to match the dimensions of labels
                    replicated_predicted_output_learn_hidden <- predicted_output_learn_hidden[[1]][, 1:ncol(labels), drop = FALSE]
                } else {
                    replicated_predicted_output_learn_hidden <- predicted_output_learn_hidden[[1]]
                }

                # Check if the number of rows in replicated_predicted_output_learn_hidden matches labels
                if (nrow(replicated_predicted_output_learn_hidden) < nrow(labels)) {
                    # If replicated_predicted_output_learn_hidden has fewer rows, replicate it to match the number of rows in labels
                    replicated_predicted_output_learn_hidden <- matrix(
                        rep(replicated_predicted_output_learn_hidden, length.out = nrow(labels) * ncol(replicated_predicted_output_learn_hidden)),
                        nrow = nrow(labels),
                        ncol = ncol(replicated_predicted_output_learn_hidden)
                    )
                } else if (nrow(replicated_predicted_output_learn_hidden) > nrow(labels)) {
                    # If replicated_predicted_output_learn_hidden has more rows, truncate it to match the number of rows in labels
                    replicated_predicted_output_learn_hidden <- replicated_predicted_output_learn_hidden[1:nrow(labels), , drop = FALSE]
                } else {
                    replicated_predicted_output_learn_hidden <- replicated_predicted_output_learn_hidden
                }

                # Calculate the error for the first layer
                error <- labels - replicated_predicted_output_learn_hidden

            } else {
                # For subsequent layers, follow your existing logic
                if (layer != self$num_layers) {
                    if (ncol(predicted_output_learn_hidden[[layer - 1]]) < ncol(predicted_output_learn_hidden[[layer]])) {
                        # Replicate previous layer output to match the dimensions of predicted_output_learn_hidden[[layer]]
                        replicated_previous_layer_output <- matrix(
                            rep(predicted_output_learn_hidden[[layer - 1]], each = ncol(predicted_output_learn_hidden[[layer]]) / ncol(predicted_output_learn_hidden[[layer - 1]])),
                            nrow = nrow(predicted_output_learn_hidden[[layer - 1]]),
                            ncol = ncol(predicted_output_learn_hidden[[layer]]),
                            byrow = FALSE  # Replicate by columns
                        )
                    } else if (ncol(predicted_output_learn_hidden[[layer - 1]]) > ncol(predicted_output_learn_hidden[[layer]])) {
                        # Truncate the previous layer output to match the dimensions of predicted_output_learn_hidden[[layer]]
                        replicated_previous_layer_output <- predicted_output_learn_hidden[[layer - 1]][, 1:ncol(predicted_output_learn_hidden[[layer]]), drop = FALSE]
                    } else {
                        replicated_previous_layer_output <- predicted_output_learn_hidden[[layer - 1]]
                    }

                    # Check if the number of rows in replicated_previous_layer_output matches predicted_output_learn_hidden[[layer]]
                    if (nrow(replicated_previous_layer_output) < nrow(predicted_output_learn_hidden[[layer]])) {
                        # If replicated_previous_layer_output has fewer rows, replicate it to match the number of rows in predicted_output_learn_hidden[[layer]]
                        replicated_previous_layer_output <- matrix(
                            rep(replicated_previous_layer_output, length.out = nrow(predicted_output_learn_hidden[[layer]]) * ncol(replicated_previous_layer_output)),
                            nrow = nrow(predicted_output_learn_hidden[[layer]]),
                            ncol = ncol(replicated_previous_layer_output)
                        )
                    } else if (nrow(replicated_previous_layer_output) > nrow(predicted_output_learn_hidden[[layer]])) {
                        # If replicated_previous_layer_output has more rows, truncate it to match the number of rows in predicted_output_learn_hidden[[layer]]
                        replicated_previous_layer_output <- replicated_previous_layer_output[1:nrow(predicted_output_learn_hidden[[layer]]), , drop = FALSE]
                    } else {
                        replicated_previous_layer_output <- replicated_previous_layer_output
                    }

                    # Calculate the error for hidden layers
                    error <- replicated_previous_layer_output - predicted_output_learn_hidden[[layer]]
                } else {
                    # For the last layer
                    if (ncol(labels) != ncol(predicted_output_learn_hidden[[layer]])) {
                        if (ncol(labels) < ncol(predicted_output_learn_hidden[[layer]])) {
                            replicated_predicted_output_learn_hidden <- predicted_output_learn_hidden[[layer]][, 1:ncol(labels), drop = FALSE]
                        } else {
                            replicated_predicted_output_learn_hidden <- matrix(
                                rep(predicted_output_learn_hidden[[layer]], each = ncol(labels) / ncol(predicted_output_learn_hidden[[layer]])),
                                nrow = nrow(predicted_output_learn_hidden[[layer]]),
                                ncol = ncol(labels),
                                byrow = FALSE
                            )
                        ...
                        replicated_predicted_output_learn_hidden <- predicted_output_learn_hidden[[layer]]
                    }

                    if (nrow(labels) != nrow(replicated_predicted_output_learn_hidden)) {
                        if (nrow(labels) < nrow(replicated_predicted_output_learn_hidden)) {
                            replicated_predicted_output_learn_hidden <- replicated_predicted_output_learn_hidden[1:nrow(labels), , drop = FALSE]
                        } else {
                            replicated_predicted_output_learn_hidden <- matrix(
        ...
        if (ncol(labels) != ncol(predicted_output_learn)) {
            if (ncol(predicted_output) < ncol(labels)) {
                # Calculate the required replication factor
                rep_factor <- ceiling((nrow(labels) * ncol(labels)) / length(predicted_output_learn))
                # Create the replicated vector and check its length
                replicated_predicted_output <- rep(predicted_output_learn, rep_factor)
                ...
                # Truncate predicted_output_learn to match the number of columns in labels
                truncated_predicted_output <- predicted_output_learn[, 1:ncol(labels)]
                # Create the matrix and check its dimensions
                predicted_output_matrix <- matrix(truncated_predicted_output, nrow = nrow(labels), ncol = ncol(labels), byrow = FALSE)
            }
        } else {
            predicted_output_matrix <- predicted_output_learn
        }

        # Calculate the error
        error <- labels - predicted_output_matrix
...
if (!is.null(errors[[1]]) && !is.null(Rdata)) {
    ...
    # Adjust rows to match Rdata
    if (nrow(errors[[1]]) != nrow(Rdata)) {
        cat("Adjusting rows in errors[[1]] to match rows in Rdata.\n")
        if (nrow(errors[[1]]) < nrow(Rdata)) {
            # Replicate rows to match Rdata
            errors[[1]] <- matrix(
            ...
            # Truncate rows to match Rdata
            errors[[1]] <- errors[[1]][1:nrow(Rdata), , drop = FALSE]
        }
    }

    # Adjust columns to match Rdata
    max_cols <- min(ncol(Rdata), ncol(errors[[1]]))
    cat("Adjusting columns in errors[[1]] to match Rdata columns.\n")
    if (max_cols > 0) {
        errors[[1]] <- errors[[1]][, 1:max_cols, drop = FALSE]
    } else {
        stop("Error: max_cols is zero or invalid. Cannot adjust columns.")
    }
...
if (ML_NN) {
    ...
    if (nrow(t(errors[[1]])) != nrow(Rdata)) {
        if (nrow(t(errors[[1]])) > nrow(Rdata)) {
            # Truncate t(errors[[1]]) to match the number of rows in Rdata
            errors[[1]] <- errors[[1]][1:nrow(Rdata), , drop = FALSE]
        } else {
            # Replicate t(errors[[1]]) to match the number of rows in Rdata
            errors[[1]] <- matrix(
                rep(errors[[1]], length.out = nrow(Rdata) * ncol(t(errors[[1]]))),
                nrow = nrow(Rdata),
                ncol = ncol(t(errors[[1]]))
            )
        }
    }

    # Check if the number of columns in t(errors[[1]]) matches the number of columns in Rdata
    if (ncol(t(errors[[1]])) != ncol(Rdata)) {
        if (ncol(t(errors[[1]])) > ncol(Rdata)) {
            # Truncate t(errors[[1]]) to match the number of columns in Rdata
            errors[[1]] <- errors[[1]][, 1:ncol(Rdata), drop = FALSE]
        } else {
            # Replicate t(errors[[1]]) to match the number of columns in Rdata
            errors[[1]] <- matrix(
    ...
    result_dim <- dim(t(error) %*% Rdata)
    weight_dim <- dim(self$weights[[layer]])

    # Update weights and biases for the first layer

    if (ncol(self$weights[[1]]) == ncol(Rdata)) {
        if (all(weight_dim == result_dim)) {
            self$weights[[1]] <- self$weights[[1]] - (lr * t(errors[[1]]) %*% Rdata)
        } else {
            print("Dimensions mismatch, handling default case.")
        }
    } else if (nrow(self$weights[[1]]) == ncol(Rdata) && ncol(self$weights[[1]]) < ncol(Rdata)) {
        self$weights[[1]] <- self$weights[[1]] - colMeans((lr * t(errors[[1]]) %*% Rdata))
    } else if (prod(weight_dim) == 1) {
        # Handle scalar weight updates
        update_value <- lr * sum(t(errors[[1]]) %*% Rdata)
        self$weights[[1]] <- self$weights[[1]] - update_value
    } else {
        self$weights[[1]] <- self$weights[[1]] - (lr * apply(t(errors[[1]]) %*% Rdata, 2, mean))
    }


    if (length(self$biases[[1]]) < length(colSums(errors[[1]]))) {
        colSums_shortened <- colSums(errors[[1]])[1:length(self$biases[[1]])]
        self$biases[[1]] <- self$biases[[1]] - (lr * colSums_shortened)
    } else if (length(self$biases[[1]]) > length(colSums(errors[[1]]))) {
        colSums_extended <- rep(colSums(errors[[1]]), length.out = length(self$biases[[1]]))
        self$biases[[1]] <- self$biases[[1]] - (lr * colSums_extended)
    } else {
        self$biases[[1]] <- self$biases[[1]] - (lr * colSums(errors[[1]]))
    }
} else {
    self$weights <- self$weights - (lr * apply(t(error) %*% Rdata, 2, mean)) # Update weights

    if (length(self$biases) < length(colSums(error))) {
        colSums_shortened <- colSums(error)[1:length(self$biases)]
        self$biases <- self$biases - (lr * colSums_shortened)
    } else if (length(self$biases) > length(colSums(error))) {
        colSums_extended <- rep(colSums(error), length.out = length(self$biases))
        self$biases <- self$biases - (lr * colSums_extended)
    } else {
        self$biases <- self$biases - (lr * colSums(error))
    }
}
...
if (ML_NN) {
    ...
    cat("Total number of layers:", self$num_layers, "\n")
    cat("Structure of hidden_outputs:\n")
    str(hidden_outputs)

    for (layer in 2:self$num_layers) {

        if (nrow(t(error)) != nrow(hidden_outputs[[layer - 1]])) {
            if (nrow(t(error)) > nrow(hidden_outputs[[layer - 1]])) {
                # Truncate t(error) to match the number of rows in hidden_outputs[[layer - 1]]
                error <- error[1:nrow(hidden_outputs[[layer - 1]]), , drop = FALSE]
            } else {
                # Replicate t(error) to match the number of rows in hidden_outputs[[layer - 1]]
                error <- matrix(
                    rep(error, length.out = nrow(hidden_outputs[[layer - 1]]) * ncol(t(error))),
                    nrow = nrow(hidden_outputs[[layer - 1]]),
                    ncol = ncol(t(error))
                )
            }
        }

        # Check if the number of columns in t(error) matches the number of columns in hidden_outputs[[layer - 1]]
        if (ncol(t(error)) != ncol(hidden_outputs[[layer - 1]])) {
            if (ncol(t(error)) > ncol(hidden_outputs[[layer - 1]])) {
                # Truncate t(error) to match the number of columns in hidden_outputs[[layer - 1]]
                error <- error[, 1:ncol(hidden_outputs[[layer - 1]]), drop = FALSE]
            } else {
                # Replicate t(error) to match the number of columns in hidden_outputs[[layer - 1]]
                error <- matrix(
                    rep(error, length.out = nrow(error) * ncol(hidden_outputs[[layer - 1]])),
                    nrow = nrow(error),
                    ncol = ncol(hidden_outputs[[layer - 1]])
                )
            }
        }
        weight_dim <- dim(self$weights[[layer]])
        result_dim <- dim(t(error) %*% hidden_outputs[[layer - 1]])

        if (ncol(self$weights[[layer]]) == ncol(hidden_outputs[[layer - 1]])) {
            cat("Dimensions match for column comparison\n")
            if (all(weight_dim == result_dim)) {
                self$weights[[layer]] <- self$weights[[layer]] - (lr * t(error) %*% hidden_outputs[[layer - 1]])
            } else {
                cat("Dimensions do not match for subtraction\n")
            }
        } else if (nrow(self$weights[[layer]]) == ncol(hidden_outputs[[layer - 1]]) && ncol(self$weights[[layer]]) < ncol(hidden_outputs[[layer - 1]])) {
            cat("Handling case where rows of weights match columns of hidden_outputs and columns of weights are less\n")
            self$weights[[layer]] <- self$weights[[layer]] - colMeans((lr * t(error) %*% hidden_outputs[[layer - 1]]))
        } else if (prod(weight_dim) == 1) {
            cat("Handling scalar weight updates\n")
            update_value <- lr * sum(t(error) %*% hidden_outputs[[layer - 1]])
            self$weights[[layer]] <- self$weights[[layer]] - update_value
        } else {
            cat("Handling generic case for mismatched dimensions\n")
            self$weights[[layer]] <- self$weights[[layer]] - (lr * apply(t(error) %*% hidden_outputs[[layer - 1]], 2, mean))
        }


        # Update biases for the current layer
        if (length(self$biases[[layer]]) == ncol(error)) {
            self$biases[[layer]] <- self$biases[[layer]] - (lr * colMeans(error))
        } else if (length(self$biases[[layer]]) < ncol(error)) {
            colMeans_shortened <- colMeans(error)[1:length(self$biases[[layer]])]
            self$biases[[layer]] <- self$biases[[layer]] - (lr * colMeans_shortened)
        } else if (length(self$biases[[layer]]) > ncol(error)) {
            colMeans_extended <- rep(colMeans(error), length.out = length(self$biases[[layer]]))
            self$biases[[layer]] <- self$biases[[layer]][1:ncol(error)] - (lr * colMeans_extended)
        } else {
            self$biases[[layer]] <- self$biases[[layer]] - (lr * colMeans(error))
        }
    ...
    if (ML_NN) {
        ...
        predicted_output_learn <- predicted_output_learn
    }
...
predict = function(Rdata, labels, activation_functions) {
    ...
    # Debugging: Validate activation function
    # Debugging step to ensure activation_functions[[1]] is valid
    if (!is.null(activation_functions[[1]])  && activation_functions[[1]] %in% c("sigmoid", "sigmoid_binary", "relu", "leaky_relu", "elu", "tanh", "swish", "softplus", "hard_sigmoid", "gelu", "selu", "mish", "maxout", "prelu", "binary_activation", "custom_activation", "custom_binary_activation", "softmax", "bent_identity")) {
        activation_function <- tryCatch(
            {
                get(activation_functions[[1]], mode = "function")
            },
            error = function(e) {
                stop("Error: Invalid activation function or it cannot be found.")
            }
        )
    } else {
    ...
    if (length(self$biases[[1]]) == 1) {
        ...
        cat("self$biases[[1]]:", self$biases[[1]], "\n")
        cat("nrow(Rdata):", nrow(Rdata), "\n")
        cat("ncol(self$weights):", as.matrix(self$weights), "\n")
    ...
    if (self$ML_NN) {
            ...
            predicted_output_predict <- tryCatch(
                {
                    ...
                    result <- tryCatch(
                        ...
                        error = function(e) {
                            stop("Activation function application failed.")
                        }
                    )
                },
                error = function(e) {
                    stop("An unexpected error occurred during the computation of the activation function.")
                }
            )
        } else {
            # No activation function provided, compute the weighted sum plus bias
            predicted_output_predict <- tryCatch(
                {
                    # Adjust biases for layer 1
                    biases <- adjust_biases_layer_1(self$biases[[1]], self$weights[[1]], Rdata)
                ...
                error = function(e) {
                    stop("An unexpected error occurred during the computation.")
                }
            )
        }
    } else {
        ...
        if (!is.null(activation_functions[[2]])) {
            ...
            # Apply dropout if dropout_rates is provided
...
# If the ML_NN flag is TRUE, calculate the outputs for subsequent layers
            ...
            if (self$ML_NN) {
                ...
                predicted_output_predict_hidden[[1]] <- hidden_outputs[[1]]



                dim_hidden_layers_predicted <- vector("list", self$num_layers)
                dim_hidden_layers_predicted[[1]] <- dim(hidden_outputs[[1]])
                ...
                for (layer in 2:self$num_layers) {
                    ...
                    # cat("Processing layer:", layer, "\n")
                    cat("hidden_outputs[[layer - 1]] dimensions outside if: ", dim(hidden_outputs[[layer - 1]]), "\n")
                    # cat("self$weights[[layer]] dimensions: ", dim(self$weights[[layer]]), "\n")
                    ...
                    if (length(self$biases[[layer]]) == 1) {
                        ...
                        cat("Debugging Info:\n")
                        cat("Layer:", layer, "\n")
                        cat("Biases:", self$biases[[layer]], "\n")
                        cat("Dimensions of hidden_outputs[[layer - 1]]:", dim(hidden_outputs[[layer - 1]]), "\n")
                        cat("hidden_outputs[[layer - 1]] dimensions: ", dim(hidden_outputs[[layer - 1]]), "\n")
                        cat("Dimensions of weights[[layer]]:", dim(self$weights[[layer]]), "\n")

                        if (is.null(hidden_outputs[[layer - 1]]) || !is.matrix(hidden_outputs[[layer - 1]])) {
                            stop("Error: hidden_outputs[[layer - 1]] is not initialized or is not a matrix.")
                        }

                        if (!is.matrix(hidden_outputs[[layer - 1]])) {
                            hidden_outputs[[layer - 1]] <- as.matrix(hidden_outputs[[layer - 1]])
                        }


                        # Ensure biases are a vector
                        bias_vector <- as.vector(self$biases[[layer]])

                        # Create biases matrix
                        biases <- matrix(rep(bias_vector, each = nrow(hidden_outputs[[layer - 1]])),
                                         nrow = nrow(hidden_outputs[[layer - 1]]),
                                         ncol = ncol(self$weights[[layer]]), byrow = TRUE)

                        # Print resulting dimensions
                        cat("Dimensions of biases matrix:", dim(biases), "\n")
                    ...
                    if (!is.null(activation_function)) {
                        hidden_outputs[[layer]] <- activation_function(hidden_outputs[[layer]])
                    } else {
                        hidden_outputs[[layer]] <- hidden_outputs[[layer]]
                    }
                    ...
                    # Ensure predicted_output_predict_hidden[[layer]] is a matrix
                    if (is.vector(predicted_output_predict_hidden[[layer]])) {
                        ...
                        print("Ensure predicted_output_predict_hidden[[layer]] is a matrix")
                    }
                }
            }else{
            ...
            end_time <- Sys.time()
                ...
                if (self$ML_NN) {
                    for (layer in 2:self$num_layers) {
                        ...
                        # print(paste("Dimensions of predicted_output_predict_hidden[[layer]]: ", dim(predicted_output_predict_hidden[[layer]])))
                        # print(paste("Class of predicted_output_predict_hidden[[layer]]: ", class(predicted_output_predict_hidden[[layer]])))

                        if (layer != self$num_layers) {
                            if (ncol(predicted_output_predict_hidden[[layer - 1]]) < ncol(predicted_output_predict_hidden[[layer]])) {
                                # For hidden layers, replicate previous layer output to match the dimensions of predicted_output_predict_hidden[[layer]]
                                replicated_previous_layer_output <- matrix(
                                    rep(predicted_output_predict_hidden[[layer - 1]], each = ncol(predicted_output_predict_hidden[[layer]]) / ncol(predicted_output_predict_hidden[[layer - 1]])),
                                    nrow = nrow(predicted_output_predict_hidden[[layer - 1]]),
                                    ncol = ncol(predicted_output_predict_hidden[[layer]]),
                                    byrow = FALSE  # Replicate by columns
                                )
                            } else if (ncol(predicted_output_predict_hidden[[layer - 1]]) > ncol(predicted_output_predict_hidden[[layer]])) {
                                # Truncate the previous layer output to match the dimensions of predicted_output_predict_hidden[[layer]]
                                replicated_previous_layer_output <- predicted_output_predict_hidden[[layer - 1]][, 1:ncol(predicted_output_predict_hidden[[layer]]), drop = FALSE]
                            } else {
                                replicated_previous_layer_output <- predicted_output_predict_hidden[[layer - 1]]
                            }

                            # Check if the number of rows in replicated_previous_layer_output matches predicted_output_predict_hidden[[layer]]
                            if (nrow(replicated_previous_layer_output) < nrow(predicted_output_predict_hidden[[layer]])) {
                                # If replicated_previous_layer_output has fewer rows, replicate it to match the number of rows in predicted_output_predict_hidden[[layer]]
                                replicated_previous_layer_output <- matrix(
                                    rep(replicated_previous_layer_output, length.out = nrow(predicted_output_predict_hidden[[layer]]) * ncol(replicated_previous_layer_output)),
                                    nrow = nrow(predicted_output_predict_hidden[[layer]]),
                                    ncol = ncol(replicated_previous_layer_output)
                                )
                            } else if (nrow(replicated_previous_layer_output) > nrow(predicted_output_predict_hidden[[layer]])) {
                                # If replicated_previous_layer_output has more rows, truncate it to match the number of rows in predicted_output_predict_hidden[[layer]]
                                replicated_previous_layer_output <- replicated_previous_layer_output[1:nrow(predicted_output_predict_hidden[[layer]]), , drop = FALSE]
                            } else {
                                # If the number of rows in replicated_previous_layer_output matches the number of rows in predicted_output_predict_hidden[[layer]], no changes are needed
                                replicated_previous_layer_output <- replicated_previous_layer_output
                            }

                            # Calculate the error
                            error_prediction <- replicated_previous_layer_output - predicted_output_predict_hidden[[layer]]
                        } else {
                            # For the last layer, ensure columns of predicted_output_predict_hidden[[layer]] match labels
                            if (ncol(labels) != ncol(predicted_output_predict_hidden[[layer]])) {
                                if (ncol(labels) < ncol(predicted_output_predict_hidden[[layer]])) {
                                    replicated_predicted_output_predict_hidden <- predicted_output_predict_hidden[[layer]][, 1:ncol(labels), drop = FALSE]
                                } else {
                                    replicated_predicted_output_predict_hidden <- matrix(
                                        rep(predicted_output_predict_hidden[[layer]], each = ncol(labels) / ncol(predicted_output_predict_hidden[[layer]])),
                                        nrow = nrow(predicted_output_predict_hidden[[layer]]),
                                        ncol = ncol(labels),
                                        byrow = FALSE
                                    )
                                ...
                                replicated_predicted_output_predict_hidden <- predicted_output_predict_hidden[[layer]]
                            }

                            if (nrow(labels) != nrow(replicated_predicted_output_predict_hidden)) {
                                if (nrow(labels) < nrow(replicated_predicted_output_predict_hidden)) {
                                    replicated_predicted_output_predict_hidden <- replicated_predicted_output_predict_hidden[1:nrow(labels), , drop = FALSE]
                                } else {
                                    replicated_predicted_output_predict_hidden <- matrix(
                                        rep(replicated_predicted_output_predict_hidden, length.out = nrow(labels) * ncol(replicated_predicted_output_predict_hidden)),
                                        nrow = nrow(labels),
                                        ncol = ncol(replicated_predicted_output_predict_hidden)
                                    )
                                }
                            }

                            # Calculate the error
                            error_prediction <- labels - replicated_predicted_output_predict_hidden
                        }
                    ...
                    if (ncol(labels) != ncol(predicted_output_predict)) {
                        if (ncol(predicted_output_predict) < ncol(labels)) {
                            ...
                            truncated_predicted_output <- predicted_output_predict[, 1:ncol(labels)]
                            # Create the matrix and check its dimensions
                            predicted_output_matrix <- matrix(truncated_predicted_output, nrow = nrow(labels), ncol = ncol(labels), byrow = FALSE)
                        }
                    } else {
...
if(ML_NN){
        ...
        # Compute the output for the first layer (not ML_NN)
        # Handle activation_functions[[1]] and activation_functions[[2]]
        hidden_output_temp <- tryCatch(
            {
                # Compute the weighted sum plus bias for the first layer
                Rdata %*% self$weights[[1]] + self$biases[[1]]
            },
            error = function(e) {
                stop("Error during first layer computation (not ML_NN): ", conditionMessage(e))
            }
        )

        # Apply activation function for activation_functions[[1]], if provided
        if (!is.null(activation_functions[[1]])) {
            hidden_output_temp <- tryCatch(
                {
                    # Dynamically fetch activation function for the input layer
                    activation_function_input <- get(activation_functions[[1]], mode = "function", envir = environment())

                    # Ensure the activation function is valid
                    if (is.null(activation_function_input)) {
                        stop("Activation function for activation_functions[[1]] is NULL.")
                    }

                    # Apply the activation function to hidden_output_temp
                    activation_function_input(hidden_output_temp)
                },
                error = function(e) {
                    stop("Error during activation function application for activation_functions[[1]]: ", conditionMessage(e))
                }
            )
        }

        # Compute the output for the second layer (first hidden layer or output layer)
        predicted_output_predict <- tryCatch(
            {
                # Compute the weighted sum plus bias for the second layer
                second_layer_output <- hidden_output_temp %*% self$weights[[2]] + self$biases[[2]]

                # Apply activation function for activation_functions[[2]], if provided
                if (!is.null(activation_functions[[2]])) {
                    # Dynamically fetch activation function for the second layer
                    activation_function_output <- get(activation_functions[[2]], mode = "function", envir = environment())

                    # Ensure the activation function is valid
                    if (is.null(activation_function_output)) {
                        stop("Activation function for activation_functions[[2]] is NULL.")
                    }

                    # Apply the activation function to the second layer output
                    activation_function_output(second_layer_output)
                } else {
                    # No activation function provided for the second layer
                    second_layer_output
                }
            },
            error = function(e) {
                stop("Error during computation for activation_functions[[2]]: ", conditionMessage(e))
            }
        )
                    ...
                    for (layer in 2:self$num_layers) {
                        ...
                        cat("Debug: Initial biases dim for layer", layer, ":\n")
                        print(dim(self$biases[[layer]]))  # Print the initial biases for debugging
                        ...
                        if (length(self$biases[[layer]]) == 1) {
                            ...
                            biases <- matrix(rep(self$biases[[layer]], times = nrow(hidden_outputs[[layer - 1]])),
                                             nrow = nrow(hidden_outputs[[layer - 1]]), ncol = ncol(self$weights[[layer]]), byrow = TRUE)
                            cat("Debug: Replicated biases matrix dim for layer", layer, ":\n")
                            print(dim(biases))
                            cat("Single bias replicated across all neurons in layer", layer, "\n")
                        } else if (length(self$biases[[layer]]) == ncol(self$weights[[layer]])) {
                            # Multiple biases, one per neuron
                ...
                if(ML_NN){
                    # Get the final error matrix for the output layer
                ...
                # Print the regularization loss for debugging
                # print(paste("Regularization loss:", reg_loss))

                # Record the loss (mean squared error) for the current epoch

                # Assuming error_last_layer and reg_loss_total are computed
                loss <- loss_function(
                    error_last_layer = error_last_layer,
                    reg_loss_total = reg_loss_total,
                    loss_type = "CrossEntropy"  # Change to your desired loss type
                )
                ...
                if(ML_NN){
                    error_before_multi <- errors[[layer]]
                }else{
                    error_before_multi <- error_1000x1
                }



                if (ncol(Rdata) != ncol(error_last_layer)) {
                    if (ncol(error_last_layer) < ncol(Rdata)) {
                        # Calculate the required replication factor
                        rep_factor <- ceiling((nrow(Rdata) * ncol(Rdata)) / length(error_last_layer))
                        # Create the replicated vector and check its length
                        replicated_error_last_layer <- rep(error_last_layer, rep_factor)
                        # Truncate the replicated vector to match the required length
                        replicated_error_last_layer <- replicated_error_last_layer[1:(nrow(Rdata) * ncol(Rdata))]
                        # Create the matrix and check its dimensions
                        error_1000x10 <- matrix(replicated_error_last_layer, nrow = nrow(Rdata), ncol = ncol(Rdata), byrow = FALSE)
                    } else {
                        # Truncate error_last_layer to match the number of columns in Rdata
                        truncated_error_last_layer <- error_last_layer[, 1:ncol(Rdata)]
                        # Create the matrix and check its dimensions
                        error_1000x10 <- matrix(truncated_error_last_layer, nrow = nrow(Rdata), ncol = ncol(Rdata), byrow = FALSE)
                    }
                } else {
                    error_1000x10 <- error_last_layer
                }
                # errors  <- replicate(self$num_layers, error_1000x10, simplify = FALSE)
                ...
                if (update_weights) {
                    ...
                    if (self$ML_NN) {
                        ...
                        for (layer in self$num_layers:self$num_layers){
                            ...
                            # Check if the number of rows in errors[[layer]] matches the number of rows in Rdata
                            # Adjust rows of errors[[layer]] to match rows of Rdata
                            if (nrow(errors[[layer]]) != nrow(Rdata)) {
                                if (nrow(errors[[layer]]) > nrow(Rdata)) {
                                    # Truncate rows to match Rdata
                                    ...
                                    # Replicate rows to match Rdata
                                    errors[[layer]] <- matrix(
                                        rep(errors[[layer]], length.out = nrow(Rdata) * ncol(errors[[layer]])),
                                        nrow = nrow(Rdata),
                                        ncol = ncol(errors[[layer]])
                                    )
                                }
                            }

                            # Adjust columns of errors[[layer]] to match columns of Rdata
                            if (ncol(errors[[layer]]) != ncol(Rdata)) {
                                cat("Debug: Dimensions of errors[[layer]] before adjustment:", dim(errors[[layer]]), "\n")
                                cat("Debug: Dimensions of Rdata:", dim(Rdata), "\n")

                                if (ncol(errors[[layer]]) > ncol(Rdata)) {
                                    # Truncate columns to match Rdata
                                    errors[[layer]] <- errors[[layer]][, 1:ncol(Rdata), drop = FALSE]
                                } else {
                                    # Replicate columns to match Rdata
                                    errors[[layer]] <- matrix(
                                ...
                                cat("Debug: Dimensions of errors[[layer]] after adjustment:", dim(errors[[layer]]), "\n")
                            }

#error before multi means before transformation to the error layers

# # Replace Rdata with (t(error_before_multi) %*% predicted_output_train_reg$predicted_output[[layer - 1]])
# new_data <- (t(error_before_multi) %*% predicted_output_train_reg$predicted_output[[layer - 1]])
# print("123")
# print(dim(t(weights_residuals)))
# # print(dim(new_data))
#
#
# # Create a new variable for the weighted errors
# t_errors_layer_pred_output <- t(errors[[layer]]) %*% predicted_output_train_reg$predicted_output[[layer - 1]]  #$$$$$$$$$$$$$
#
# # Create a new variable for the transpose of weights_residuals
# t_weights_residuals <- t(weights_residuals)
# print("dim t w r")
# print(dim(t_weights_residuals))  # Print dimensions of t_weights_residuals for debugging
# print("dim t_errors_layer_pred_output")
# print(dim(t_errors_layer_pred_output))  # Print dimensions of t_errors_layer_pred_output for debugging
#
# # Adjust the dimensions of t_weights_residuals to match t_errors_layer_pred_output
# # Reshape t_weights_residuals to match the dimensions of t_errors_layer_pred_output
# target_rows <- nrow(t_errors_layer_pred_output)
# target_cols <- ncol(t_errors_layer_pred_output)
#
# # Replicate and reshape t_weights_residuals to match target dimensions
# t_weights_residuals <- matrix(
#     rep(t_weights_residuals, length.out = target_rows * target_cols),
#     nrow = target_rows,
#     ncol = target_cols
# )
#
# # Final check to ensure dimensions match
# print("Adjusted dim t w r")
# print(dim(t_weights_residuals))  # Print adjusted dimensions of t_weights_residuals for debugging
# print("Adjusted dim t_errors_layer_pred_output")
# print(dim(t_errors_layer_pred_output))  # Print adjusted dimensions of t_errors_layer_pred_output for debugging
                            ...
                            # Compute the gradient matrix for the current layer
                             if (layer == 1) {
                                 # For the first layer, use input_data
                                grads_matrix <- t(errors[[layer]]) %*% Rdata
...
#                                 print("dim t w r")
#                                 print(dim(t(weights_residuals)))
# print("dim (t(errors[[layer]]) %*% predicted_output_train_reg$predicted_output[[layer - 1]])")
#                             print(dim((t(errors[[layer]]) %*% predicted_output_train_reg$predicted_output[[layer - 1]])))
#                             # grads_matrix <<- t_weights_residuals * (new_data)
#
# grads_matrix <<- t_weights_residuals * (t(errors[[layer]]) %*% predicted_output_train_reg$predicted_output[[layer - 1]])
#                             print(grads_matrix)
#     # grads_matrix <<- t(weights_residuals %*% errors[[layer]]) %*% predicted_output_train_reg$predicted_output[[layer - 1]]
# print(" dim predicted_output_train_reg$predicted_output[[layer - 1]]")
# print(dim(predicted_output_train_reg$predicted_output[[layer - 1]]))
# grads_matrix2 <<- t(errors[[layer]]) %*% predicted_output_train_reg$predicted_output[[layer - 1]]
#
# }
                             }else {
    # For other layers, use the output from the previous layer
    grads_matrix <<- t(errors[[layer]]) %*% predicted_output_train_reg$predicted_output[[layer - 1]]
    print(grads_matrix)
                    ...
                    if (reg_type == "L1") {
                        ...
                        print(self$weights)
                        print("3")
                        print(as.matrix(self$lambda))
                        print("4")
                        print(as.matrix(self$weights))

                        self$lambda <- as.numeric(self$lambda)
                        self$weights <- as.matrix(as.numeric(self$weights))
                        ...
                        if (!is.null(optimizer_params_biases[[layer]]) && !is.null(optimizer)) {
                            if (optimizer == "adam") {
                                ...
                                if (is.matrix(updated_biases_update) && identical(dim(self$biases[[layer]]), dim(t(updated_biases_update)))) {
                                    ...
                                    if (ncol(as.matrix(self$biases)) == ncol(error_1000x10)) {
                                        self$biases <- self$biases - lr * (error_1000x10)
                                    } else {
...
train = function(Rdata, labels, lr, ensemble_number, num_epochs, threshold, reg_type, numeric_columns, activation_functions_learn, activation_functions, dropout_rates_learn, dropout_rates, optimizer, beta1, beta2, epsilon, lookahead_step, batch_normalize_data, gamma_bn = NULL, beta_bn = NULL, epsilon_bn = 1e-5, momentum_bn = 0.9, is_training_bn = TRUE, shuffle_bn = FALSE) {
    ...
    Rdata$DEATH_EVENT <- as.factor(labels)
    Rdata$DEATH_EVENT <- as.factor(Rdata$DEATH_EVENT)

    # Step 2: Train the Random Forest model
    rf_model <- randomForest(
        DEATH_EVENT ~ .,         # Target variable
        data = Rdata,            # Training data
        ntree = 100,             # Number of trees
        importance = TRUE,       # Calculate feature importance
        mtry = sqrt(ncol(Rdata) - 1)  # Number of variables tried at each split
    )
    ...
    # Step 4: Rank and select features based on a threshold
    importance_df <- as.data.frame(importance_rf_model)
    importance_df$Feature <- rownames(importance_rf_model)

    # Define a threshold for feature importance
    threshold <- 25  # Adjust based on your preference

    # Identify less important features
    less_important_features <- importance_df$Feature[importance_df$MeanDecreaseAccuracy < threshold]

    # Step 5: Zero out less important features
    for (feature in less_important_features) {
        Rdata[[feature]] <- 0
    }


    Rdata <- as.matrix(Rdata[, !names(Rdata) %in% "DEATH_EVENT"])
            ...
            if (never_ran_flag == TRUE) {
                ...
                for (i in 1:length(self$ensemble)) {
                   ...
                   #    # predicted_outputAndTime$loss_status <- 'exceeds_10000'
                        ...
                        # residuals <- labels - predicted_outputAndTime$predicted_output_l2$predicted_output[[num_layers]]

                        # Identify correctly predicted instances
                        # correctly_predicted_indices <- which(predicted_outputAndTime$predicted_output_l2$predicted_output[[num_layers]] == labels)



                        residuals <- labels - predicted_outputAndTime$predicted_output_l2$predicted_output[[num_layers]]
                        weights_residuals <<- ifelse(predicted_outputAndTime$predicted_output_l2$predicted_output[[num_layers]] == labels, 1, 200)  # Example: Assign 2 to incorrect predictions



                        # Rdata_filtered <<- Rdata[-correctly_predicted_indices, ]
                        # labels_filtered <<- labels[-correctly_predicted_indices]

                        # updated_labels2 <<- labels - residuals
                        # Ensure Rdata is a data frame
                        ####################################################
                        #this is for inspecting data
                        ####################################################
                        # Rdata <- as.data.frame(Rdata)
                        # Rdata <- cbind(Rdata, labels)
                        # # # Add the predictions and residuals columns using mutate
                        #
                        #
                        #
                        #
                        ...
                        #            Residuals = residuals, softplus_output = softplus_output)
                        # #
                        # write_xlsx(Rdata_predictions, "Rdata_predictions_NULL.xlsx")
                        #
                        # library(writexl)
                        #
                        # # Specify the path
                        # file_path <<- "C:/Users/wfky1/OneDrive/Documents/R/CopyOfDESONN11_20240622/Rdata_predictions4.xlsx"
                        #
                        ...
                        output_directory <- "C:/Users/wfky1/OneDrive/Documents/R/CopyOfDESONN11_20240622/"

                        # Loop through each layer output
                        for (l in seq_along(all_layer_outputs)) {
                            layer_output <- all_layer_outputs[[l]]
                            layer_weights <- dim_hidden_sizes[[l]][2]  # Assume this gives weight info per layer

                            # Combine outputs and weights for storage or visualization
                            combined_layer_data <- data.frame(
                                Output = as.vector(layer_output),
                                Weight = layer_weights
                            )

                            # Create the file name for each layer
                            file_name <- paste0(output_directory, "Layer_", l, "_data.xlsx")

                            # Save the layer-specific data to the specified directory
                            write_xlsx(combined_layer_data, file_name)
                        }
...
#     # Apply the softplus function to input z
#     softplus_output <<- z #log(1 + exp(z))  # log1p(exp(z))
#     softplus_output2 <<- log(1 + exp(z))
#     # Return 0 if softplus_output is between 0.76 and 0.79, else return 1
#     return(ifelse(softplus_output > 0.76 & softplus_output < 0.79, 0, 1))
# }
...
custom_activation <- function(z) {
    ...
    # Apply the softplus function to input z
    softplus_output <- log(1 + exp(z)) #ifelse(z > 0, z, 0) #log(1 + exp(z)) #log1p(exp(z))

    # Compute quantiles from all_outputs
    quantiles <- quantile(softplus_output, probs = seq(0, 1, by = 0.01))

    # Determine the threshold for the 70th percentile
    threshold_79th <- quantiles["79%"]

    # Use the threshold to determine activation
    return(ifelse(softplus_output > threshold_79th, 1, 0))
}
...
#
#     return(ifelse(softplus_output > 0.51, 1, 0)) #0.00000000001
...
loss_function <- function(error_last_layer, reg_loss_total, loss_type) {
    if (loss_type == "MSE") {
        # Mean Squared Error
        loss <- mean(error_last_layer^2)
    } else if (loss_type == "MAE") {
        # Mean Absolute Error
        loss <- mean(abs(error_last_layer))
    } else if (loss_type == "CrossEntropy") {
        # Binary Cross-Entropy Loss
        epsilon <- 1e-15  # Small value to avoid log(0)
        error_last_layer <- pmax(pmin(error_last_layer, 1 - epsilon), epsilon)  # Clip values to avoid numerical issues
        loss <- -mean(log(error_last_layer))  # BCE simplified to directly handle logits
    } else if (loss_type == "CategoricalCrossEntropy") {
        # Categorical Cross-Entropy Loss (Multi-Class Classification)
        epsilon <- 1e-15  # Small value to avoid log(0)
        error_last_layer <- pmax(pmin(error_last_layer, 1 - epsilon), epsilon)  # Clip values
        loss <- -mean(rowSums(log(error_last_layer)))
    } else {
...
quantization_error <- function(SONN, Rdata, run_id) {
    ...
    # Ensure both SONN$weights and Rdata are numeric matrices
        ...
        if (!is.matrix(Rdata) || !is.matrix(SONN)) {
            if(!is.matrix(Rdata)){
                Rdata <- as.matrix(Rdata)
                ...
                stop("Both SONN and Rdata must be matrices.")
            }
        ...
        if (!is.matrix(SONN) || !is.matrix(Rdata)) {
            ...
            if (!is.numeric(SONN) || !is.numeric(Rdata)) {
                if(!is.numeric(Rdata)){
                    ...
                    stop("Both SONN and Rdata must be numeric matrices.")
                }
            }}
}
