...
# - Provide helpers to list/select a specific model
...
.hard_stop <- function(msg = "[prepare_disk_only] Done; stopping script.") {
  cat(msg, "\n")
  if (interactive()) {
    # In RStudio: stop evaluating this script, don't kill the session
...
# ===== config =========================================================
.BM_DIR <- "artifacts"                          # where RDS files live
...
# Preference scoring (for auto-pick)
...
# Ensure one metadata object is present in env (prefer env; else load from RDS)
bm_ensure_loaded <- function(
  ...
  # else pick best/newest from RDS
...
# ===== PHASE 1: snapshot to RDS and wipe env ==========================
bm_prepare_disk_only <- function(
  ...
  # ensure there is a metadata object in env or disk we can save
  bm_ensure_loaded(name_hint, prefer_kind, prefer_ens, prefer_model, dir = dir)
  
  # detect chosen symbol name from env for consistent filename
  env_df <- bm_list_env()
  ...
  # 2) wipe environment completely (RAM)
  bm_clear_env_except(keep_names = character(0))
  
  invisible(rds_path)
}

# ===== explicit selector for multiple models ==========================
bm_select_exact <- function(kind = "Main", ens = 0L, model = 1L, dir = .BM_DIR) {
  # Prefer env
  env <- bm_list_env()
  hit <- subset(env, kind == kind & ens == ens & model == model)
  if (nrow(hit)) return(get(hit$name, envir = .GlobalEnv))
  # Else check RDS
  rds <- bm_list_rds(dir)
  hit <- subset(rds, kind == kind & ens == ens & model == model)
  if (nrow(hit)) return(readRDS(hit$path))
  stop(sprintf("Model not found: kind=%s ens=%d model=%d", kind, ens, model))
}

# ===== convenience auto-find used by your !train block ================
.auto_find_meta <- function() {
  env_df <- bm_list_env()
  if (!nrow(env_df)) return(NULL)
  chosen <- bm_pick(env_df, prefer_kind = c("Main","Temp"), prefer_ens = c(0L,1L), prefer_model = 1L)
  get(chosen$name, envir = .GlobalEnv)
}

# ===== record extractor (adapt to your metadata schema) ===============
# Expects 'meta$records[[i]]$weights' and '$biases' shapes you saved at train time.
extract_best_records <- function(meta, ML_NN, model_index = 1L) {
  # model_index = which SONN inside DESONN you want (1-based)
  rec <- meta$records[[as.integer(model_index)]]
  if (is.null(rec)) stop(sprintf("No record for model_index=%d", as.integer(model_index)))
  list(weights = rec$weights, biases = rec$biases)
}
